...
class ResultService {
	...
	def logResult = { consentAgreementInstance, linkedResult, appName, username ->
		...
		procedure = "EXEC ${dbPrefixName}.mr_result @sid=?, @ResultCode=?, @ReceivedDate=?, @CreatedByWhat=?, @username=??"
		//procedure = "EXEC ${dbPrefixName}.Transact (?, ?, ?, ?, ?, ?)"
	   
		sql.call(procedure, [consentAgreementInstance, linkedResult, now, appName, username])
